...
public class ModelManager extends ComponentManager implements Model {
    ...
    public synchronized int deleteTask(ReadOnlyTask target) throws TaskNotFoundException {
        ...
        indicateTaskBookChanged();
        return getIndexOfTask((Task)target);
    }
	...
	@Override
	public synchronized int updateTask(Task toReplace, Task toUpdate) throws DateClashTaskException {
        taskBook.updateTask(toReplace, toUpdate);
        updateFilteredListToShowAll();
        sortedTasks = new SortedList<>(filteredTasks, new TaskComparator());
        indicateTaskBookChanged();	
        return getIndexOfTask(toUpdate);
	}
